...
const TrackingPanel = () => {
  ...
  // ✅ CAMBIO CRÍTICO: Usar updateNCInFirebase en lugar de updateNCStatus
  const handleStatusChange = async (newStatus) => {
    if (!selectedNC) return;
    
    try {
      // ✅ SOLUCION: Usar la función Firebase en lugar de la local
      await helpers.updateNCInFirebase(selectedNC.id, { status: newStatus });
  ...
  // Sort NCs with safe date comparison
            ...
            >
              ...
              {sortedNCs.map(nc => (
                <option key={nc.id} value={nc.id}>
                  ...
                  {nc.description.length > 50 ? '...' : ''} ({nc.status})
                </option>
              ))}
            </select>
          </div>
          ...
          <>
            ...
            <div className="nc-tracking-details">
              <div className="nc-overview-card">
                <div className="nc-status-header">
                  <h4 className="nc-subsection-title">
                    {selectedNC.number} - {selectedNC.project}
                  </h4>
                  <div className="nc-status-badges">
                    <span className={`nc-status-badge nc-status-${selectedNC.status}`}>
                      {selectedNC.status?.toUpperCase()}
                    </span>
                    <span className={`nc-priority-badge nc-priority-${selectedNC.priority}`}>
